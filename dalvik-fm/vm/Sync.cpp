...
void dvmSweepMonitorList(Monitor** mon, int (*isUnmarkedObject)(void*))
{
    ...
    while (curr != NULL) {
        ...
        if (obj != NULL && !dvmIsClassObject(obj) && (*isUnmarkedObject)(obj) != 0) {
#else
...
static bool unlockMonitor(Thread* self, Monitor* mon)
{
    ...
    if (mon->owner == self) {
         ...
         */
#ifdef _DEBUG
		fastiva_debug_break("unlock of unowned monitor", true);
#endif
...
void dvmLockObject(Thread* self, Object *obj)
{
    ...
    assert(self != NULL);
    assert(obj != NULL);
    assert(dvmIsClassObject(obj->clazz));
#endif
                    ...
                    if (LW_LOCK_OWNER(thin) == 0) {
                        ...
                        if (sleepDelayNs == 0) {
                            ...
                            if (sleepDelayNs < maxSleepDelayNs / 2) {
                                ...
                                sleepDelayNs = minSleepDelayNs;
#ifdef _DEBUG
		ALOGD("thread(%d)[%d]: locked by (%d)", self->threadId, dvmGetSysThreadId(), LW_LOCK_OWNER(thin));
		assert(self->threadId < 0xFFFFFF);
#endif
                            }
                        }
                    }
...
u4 dvmIdentityHashCode(Object *obj)
{
	...
	Thread* self = dvmThreadSelf()
	if (self != NULL) {
		fastiva_dvmIdentityHashCode(obj);
	}
	else {
		fastiva_dvmIdentityHashCode(obj);
	}
...
u4 fastiva_dvmIdentityHashCode(Object *obj)
{
	Not supported in Fastiva !!
#endif
    Thread *self, *thread;
    volatile u4 *lw;
    size_t size;
