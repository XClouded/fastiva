    ...
    Object* loader)
{
...
#ifdef FASTIVA
    size_t newClassSize =
        sizeof(fastiva_Class) + kProxySFieldCount * sizeof(StaticField);
#else
    size_t newClassSize =
    ...
    ClassObject* newClass =
        (ClassObject*) fastiva_dvmMalloc(newClassSize, ALLOC_NON_MOVING, gDvm.classJavaLangClass);
#else
    ClassObject* newClass =
...
#ifdef FASTIVA
        size_t interfacesSize = sizeof(InterfaceEntry) * interfaceCount;
        newClass->iftable =
            (InterfaceEntry*)dvmLinearAlloc(newClass->classLoader, interfacesSize);
        for (size_t i = 0; i < interfaceCount; i++)
          newClass->iftable[i].clazz = ifArray[i];
...
#ifndef __GNUC__
    const ClassObject** mixSet = (const ClassObject**)alloca(sizeof (void*) * mixLen);
#else
...
#ifdef FASTIVA
    dstMeth->accessFlags |= ACC_VIRTUAL$;
#endif
