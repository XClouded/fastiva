...
#include <java/lang/reflect/Method.inl>
#include <java/lang/reflect/Constructor.inl>
#endif
...
static Object* createFieldObject(Field* field, const ClassObject* clazz)
{
...
#ifdef FASTIVA
    StringObject* JPP_VOLATILE_STACK_VAR nameObj = NULL;
#else
	...
	{ TRY$ {
		...
		java_lang_reflect_Field_I$::init$((java_lang_reflect_Field_p)fieldObj, 
										  (java_lang_Class_p)clazz, 
										  (java_lang_Class_p)type, 
										  (java_lang_String_p)nameObj, 
										  slot, field_idx);
#else
		java_lang_reflect_Field_I$::init$((java_lang_reflect_Field_p)fieldObj, 
										  (java_lang_Class_p)clazz, 
	...
	CATCH_ANY$ {
		dvmSetException(dvmThreadSelf(), (Object*)catched_ex$);
...
#ifdef FASTIVA
    if ((meth->accessFlags & ACC_VIRTUAL$) == 0) {//dvmIsDirectMethod(meth) || dvmIsFinalMethod(meth)) {
#else
...
static Object* createConstructorObject(Method* meth)
{
    Object* result = NULL;
#ifdef FASTIVA
    ArrayObject* JPP_VOLATILE_STACK_VAR params = NULL;
    ArrayObject* JPP_VOLATILE_STACK_VAR exceptions = NULL;
#else
	...
	{ TRY$ {
#if FASTIVA_TARGET_ANDROID_VERSION >= 40400
		java_lang_reflect_Constructor_I$::init$((java_lang_reflect_Constructor_p)consObj, 
												(java_lang_Class_p)meth->clazz, 
												(java_lang_Class_ap)params, 
												(java_lang_Class_ap)exceptions, 
												slot, method_idx);
#else
		java_lang_reflect_Constructor_I$::init$((java_lang_reflect_Constructor_p)consObj, 
												(java_lang_Class_p)meth->clazz, 
...
Object* dvmCreateReflectMethodObject(const Method* meth)
{
	...
	{ TRY$ {
#if FASTIVA_TARGET_ANDROID_VERSION >= 40400
		java_lang_reflect_Method_I$::init$((java_lang_reflect_Method_p)methObj, 
											(java_lang_Class_p)meth->clazz, 
											(java_lang_Class_ap)params, 
											(java_lang_Class_ap)exceptions, 
											(java_lang_Class_p)returnType,
											(java_lang_String_p)nameObj, 
											slot, method_idx);
#else
		java_lang_reflect_Method_I$::init$((java_lang_reflect_Method_p)methObj, 
    ...
    const char* name, const char* parameterDescriptors)
{
    ...
    //fastiva_debug_break("DDDDD", false);
    return NULL;
}
...
ArrayObject* dvmGetInterfaces(ClassObject* clazz)
{
	...
	for (uint i = 0; i < count; i ++) {
		((ClassObject**)(void*)interfaceArray->contents)[i] = clazz->iftable[i].clazz;
	}
#else
...
#if defined FASTIVA
		// @zee 8byte align;
		((java_lang_Long_p)wrapperObj)->set__value(value.j);
#else
        *(s8*)dataPtr = value.j;
#endif
    else
