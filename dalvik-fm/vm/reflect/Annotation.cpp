...
#define FASTIVA_DUMP_LOG()		//dvmDumpThread(dvmThreadSelf(), false);
#endif
...
#ifdef FASTIVA
	FASTIVA_ASSERT(d2f_isFastivaClass(clazz) == 0);
 ...
 */
#ifdef FASTIVA
ArrayObject* g_emptyAnnoArray = NULL;
#else
static 
#endif
...
{
#ifdef FASTIVA
	if (g_emptyAnnoArray == NULL) {
		g_emptyAnnoArray = dvmAllocArrayByClass(
			gDvm.classJavaLangAnnotationAnnotationArray, 0, ALLOC_DEFAULT);
		void fastiva_addGlobalReference(Object* obj);
		fastiva_addGlobalReference(g_emptyAnnoArray);
	}
	else {
		dvmAddTrackedAlloc(g_emptyAnnoArray, NULL);
	}
	return g_emptyAnnoArray;
#endif
...
#if 0 //def _DEBUG
#define LOGDD ALOGD
ArrayObject* DLOG(ArrayObject* anno) {
	if (anno != NULL) {
		for (int i = anno->length; --i >= 0; ) {
			Object* o = ((java_lang_Object_ap)anno)->get$(i);
			if (dvmIsArrayClass(o->clazz)) {
				DLOG((ArrayObject*) o);
			}
			else {
				java_lang_System_C$::getRawStatic$()->get__err()->println_(o->toString_());
			}
		}
	}
	return anno;
}
Object* DLOG(Object* anno) {
	if (anno != NULL) {
		if (dvmIsArrayClass(anno->clazz)) {
			DLOG((ArrayObject*) anno);
		}
		else {
			java_lang_System_C$::getRawStatic$()->get__err()->println_(anno->toString_());
		}
	}
	return anno;
}
ClassObject* DLOG(ClassObject* anno) {
	//if (anno != NULL) 
	//java_lang_System_C$::getRawStatic$()->get__err()->println_(anno->toString_());
	return anno;
}
bool DLOG(const ClassObject* anno, bool res) {
	if (anno != NULL) 
	java_lang_System_C$::getRawStatic$()->get__err()->print_(((fastiva_Class_p)anno)->toString_());
	java_lang_System_C$::getRawStatic$()->get__err()->println_((jbool)res);
	return res;
}
#else 
#define LOGDD(...)  // ignore
...
ArrayObject* dvmGetClassAnnotations(const ClassObject* clazz)
{
	...
	LOGDD("%s, %s", __FUNCTION__, clazz->descriptor);

#ifdef FASTIVA
	if (d2f_isFastivaClass(clazz) != 0) {
		ArrayObject* annos = d2f_getClassAnnotations(clazz);
		if (annos == NULL) {
			annos = emptyAnnoArray();
		}
		else {
			dvmAddTrackedAlloc(annos, NULL);
		}
		return DLOG(annos);
	}
#endif
...
Object* dvmGetClassAnnotation(const ClassObject* clazz,
        const ClassObject* annotationClazz)
{
	LOGDD("%s, %s, %s", __FUNCTION__, clazz->descriptor, annotationClazz->descriptor);

#ifdef FASTIVA
	if (d2f_isFastivaClass(clazz) != 0) {
		// no addTrackeAlloc
		return DLOG(d2f_getClassAnnotation(clazz, annotationClazz));
	}
#endif
    ...
    return DLOG(getAnnotationObjectFromAnnotationSet(clazz, pAnnoSet,
...
#ifdef FASTIVA
	if (d2f_isFastivaClass(clazz) != 0) {
		return DLOG(annotationClazz, d2f_getClassAnnotation(clazz, annotationClazz) != NULL);
	}
#endif
    const DexAnnotationSetItem* pAnnoSet = findAnnotationSetForClass(clazz);
    if (pAnnoSet == NULL) {
        return DLOG(annotationClazz, false);
    }
    const DexAnnotationItem* pAnnoItem = getAnnotationItemFromAnnotationSet(
            clazz, pAnnoSet, kDexVisibilityRuntime, annotationClazz);
    return DLOG(annotationClazz, (pAnnoItem != NULL));
}
...
#ifdef FASTIVA
	if (d2f_isFastivaClass(clazz) != 0) {
		ArrayObject* m = d2f_getClassSignatureAnnotation(clazz);
		if (m != NULL) {
			dvmAddTrackedAlloc(m, NULL);
		}
		return DLOG(m);
	}
#endif
...
#ifdef FASTIVA
	if (d2f_isFastivaClass(clazz) != 0) {
		Object* m = d2f_getEnclosingMethod(clazz);
		if (m != NULL) {
			dvmAddTrackedAlloc(m, NULL);
		}
		return DLOG(m);
...
#ifdef FASTIVA
	if (d2f_isFastivaClass(clazz) != 0) {
		ClassObject* c = d2f_getDeclaringClass(clazz);
		if (c != NULL) {
			dvmAddTrackedAlloc(c, NULL);
		}
		return DLOG(c);
	}
#endif
...
#ifdef FASTIVA
	if (d2f_isFastivaClass(clazz) != 0) {
		ClassObject* c = d2f_getEnclosingClass(clazz);
		if (c != NULL) {
			dvmAddTrackedAlloc(c, NULL);
		}
		return DLOG(c);
...
#ifdef FASTIVA
	if (d2f_isFastivaClass(clazz) != 0) {
		*pName = d2f_getInnerClassName(clazz);
		if (*pName != NULL) {
			dvmAddTrackedAlloc((Object*)*pName, NULL);
			*pAccessFlags = clazz->accessFlags; // @zee @todo must support it??
			return true;
		}
		return false;
	}
...
#ifdef FASTIVA
	if (d2f_isFastivaClass(clazz) != 0) {
		ArrayObject* v = d2f_getDeclaredClasses(clazz);
		if (v != NULL) {
			dvmAddTrackedAlloc(v, NULL);
		}
		return v;
	}
#endif
...
ArrayObject* dvmGetMethodAnnotations(const Method* method)
{
	...
	LOGDD("%s, %s::%s", __FUNCTION__, method->clazz->descriptor, method->name);
#ifdef FASTIVA
	if (d2f_isFastivaClass(clazz) != 0) {
		ArrayObject* v = d2f_getMethodAnnotations(method);
		if (v != NULL) {
			dvmAddTrackedAlloc(v, NULL);
		}
		return DLOG(v);
...
Object* dvmGetMethodAnnotation(const ClassObject* clazz, const Method* method,
        const ClassObject* annotationClazz)
{
	LOGDD("%s, %s::%s, %s", __FUNCTION__, method->clazz->descriptor, method->name, annotationClazz->descriptor);
#ifdef FASTIVA
	if (d2f_isFastivaClass(clazz) != 0) {
		Object* v = d2f_getMethodAnnotation(method, annotationClazz);
		if (v != NULL) {
			dvmAddTrackedAlloc(v, NULL);
		}
		return DLOG(v);
...
#ifdef FASTIVA
	if (d2f_isFastivaClass(clazz) != 0) {
		return DLOG(annotationClazz, d2f_getMethodAnnotation(method, annotationClazz) != NULL);
	}
#endif
...
#ifdef FASTIVA
	if (d2f_isFastivaClass(clazz) != 0) {
		ArrayObject* m = d2f_getMethodSignatureAnnotation(method);
		if (m != NULL) {
			dvmAddTrackedAlloc(m, NULL);
		}
		return DLOG(m);
...
ArrayObject* dvmGetMethodThrows(const Method* method)
{
	...
	if (d2f_isFastivaClass(clazz) != 0) {
		//static int cntHit = 0;
		//ALOGE("Check stack dvmGetMethodThrows %s->%s", pContext->m_pJNI->m_signature, method->name);
		return NULL;
	}
#endif
...
ArrayObject* dvmGetFieldAnnotations(const Field* field)
{
	...
	LOGDD("%s, %s::%s", __FUNCTION__, field->clazz->descriptor, field->name);
#ifdef FASTIVA
	if (d2f_isFastivaClass(clazz) != 0) {
		ArrayObject* v = d2f_getFieldAnnotations(field);
		if (v != NULL) {
			dvmAddTrackedAlloc(v, NULL);
		}
		return DLOG(v);
...
Object* dvmGetFieldAnnotation(const ClassObject* clazz, const Field* field,
        const ClassObject* annotationClazz)
{
	LOGDD("%s, %s::%s, %s", __FUNCTION__, field->clazz->descriptor, field->name, annotationClazz->descriptor);
#ifdef FASTIVA
	if (d2f_isFastivaClass(clazz) != 0) {
		Object* v= d2f_getFieldAnnotation(field, annotationClazz);
		if (v != NULL) {
			dvmAddTrackedAlloc(v, NULL);
		}
		return DLOG(v);
        ...
        const Field* field, const ClassObject* annotationClazz)
{
	...
	if (d2f_isFastivaClass(clazz) != 0) {
		// no addTrackedAlloc
		return DLOG(annotationClazz, d2f_getFieldAnnotation(field, annotationClazz) != NULL);
	}
#endif
...
ArrayObject* dvmGetFieldSignatureAnnotation(const Field* field)
{
...
#ifdef _DEBUG
	ALOGD("Finding field_sig %s.%s", field->clazz->descriptor, field->name);
#endif

#ifdef FASTIVA
	if (d2f_isFastivaClass(clazz) != 0) {
#ifdef _DEBUG
		//assert(0 != strcmp(field->name, "versions"));
#endif
		ArrayObject* v = d2f_getFieldSignatureAnnotation(field);
		if (v != NULL) {
			dvmAddTrackedAlloc(v, NULL);
		}
		return DLOG(v);
	...
	if (signature != NULL) {
		//assert(0 != strcmp(field->name, "versions"));
		java_lang_String_ap values = (java_lang_String_ap)signature;
		for (int i = 0; i < values->length(); i ++) {
			java_lang_String_p str = values->get$(i);
			void d2f_getBytes(char* buff, int max_len, java_lang_String_p str);
			char buf[1024];
			d2f_getBytes(buf, 1024, str);
			ALOGD("found value %d = %s", i, buf);
		}
	}
#endif
    return DLOG(signature);
...
#ifdef FASTIVA
	if (d2f_isFastivaClass(clazz) != 0) {
		ArrayObject* v = d2f_getParameterAnnotations(method);
		if (v != NULL) {
			dvmAddTrackedAlloc(v, NULL);
		}
		else {
			v = emptyAnnoArrayArray(countMethodArguments(method));
		}
		return DLOG(v);
	}
#endif
