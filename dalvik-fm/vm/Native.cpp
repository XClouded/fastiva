    ...
    const Method* method, Thread* self)
{
...
#ifdef FASTIVA
        ((Method*)method)->nativeFunc = dfunc;
#else
    ...
    IF_ALOGW() {
        ...
        ALOGW("No implementation found for native %s.%s:%s",
            clazz->descriptor, method->name, desc);
        fastiva_debug_break("No implementation found", true);
        free(desc);
    }
...
#ifdef FASTIVA
typedef int (*OnLoadFunc)(JavaVM*, fastiva_InitArgs*);
#else
...
#ifdef FASTIVA
			fastiva_InitArgs args = { (java_lang_ClassLoader_p)classLoader, /*pathName,*/ &fastiva_RuntimeProxy::g_instance };
            version = (*func)(gDvmJni.jniVm, &args);
#else
