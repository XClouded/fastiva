...
void dvmAddToSuspendCounts(Thread* thread, int delta, int dbgDelta)
{
    ...
    int lastCount = thread->suspendCount;
	void fastiva_dvmAddToSuspendCounts_unsafe(Thread* thread, int delta, int dbgDelta);
	fastiva_dvmAddToSuspendCounts_unsafe(thread, delta, dbgDelta);

	if (delta > 0 && lastCount == 0 && thread->m_pNativeStackPointer == NULL) {
		void fastiva_sendSuspendRequest(pthread_t);
		fastiva_sendSuspendRequest(thread->handle);
	}
}
