...
extern void d2f_executeMethod(Thread* self, const Method* method, bool fromJni, JValue* pResult);
#endif
/*
...
#ifdef FASTIVA
bool fastiva_dvmPushInterpFrame(Thread* self, const Method* method) {
	return dvmPushInterpFrame(self, method);
}
#endif
/*
...
#ifdef FASTIVA
void fastiva_dvmPopFrame(Thread* self) {
	dvmPopFrame(self);
}
#endif
    ...
    bool checkAccess)
{
	...
	if (g_fastiva_enable_call_log) {
	ALOGE("$$$ calling th=%d (%s.%s:%s):", self->threadId,
    ...
    if (checkAccess) {
        ...
        if (!dvmCheckMethodAccess(d2f_getCallerClass(self),
                method))
#else
		// @zee A caller Method -> Method.invoke() -> here
...
{
	if (method->accessFlags & ACC_FASTIVA_CPP_NO_JNI_STACK_SHIFT) {
		FASTIVA_ASSERT(!d2f_inFastivaFNI(self));
		d2f_executeJppMethod(obj, method, pResult, args);
		return;
	}
...
#ifdef FASTIVA
	d2f_executeMethod(self, method, fromJni, pResult);
#else
    if (dvmIsNativeMethod(method)) {
    ...
    bool noAccessCheck)
{
    ...
    for (int i = 0; i < argListLength; i++) {
		...
		fastiva_getFieldOffset((u1*)*args, 4);
#endif
...
#ifdef FASTIVA
	d2f_executeMethod(self, method, false, &retval);
#else
    if (dvmIsNativeMethod(method)) {
        TRACE_METHOD_ENTER(self, method);
        /*
