...
#ifdef FASTIVA
static bool isEmptyDexFile = false;
#endif
...
#if defined FASTIVA && defined G_WATCH
		if (chmod(dirName.c_str(), 777) < 0)
#endif
		{
...
#ifndef FASTIVA_NO_SEPARATE_OPT_PROCESS
    pid = fork();
    if (pid == 0) {
#endif
...
#ifdef _WIN32
        const char** argv = (const char**)alloca(sizeof(char*) * (argc+1));             // last entry is NULL
		typedef char ARG_VALUE[kMaxIntLen];
		ARG_VALUE* values = (ARG_VALUE*)alloca(sizeof(ARG_VALUE)*argc);
#else
        ...
        ClassPathEntry* cpe;
#ifdef FASTIVA
        for (cpe = gDvm.bootClassPath; cpe->fileName != NULL; cpe++) {
#else
...
#ifdef FASTIVA_NO_SEPARATE_OPT_PROCESS
		{
			int dexopt_main(int argc, char* const argv[]);
		dexopt_main(curArg, (char* const*)argv);
		}
#ifdef _WIN32
			_exit(0);
#endif
    ...
    const char* fileName, u4 modWhen, u4 crc, bool isBootstrap)
{
...
#ifdef _DEBUG
    ALOGD("$$$ optimizing (%s, isb=%d)", fileName, isBootstrap);
#else
...

    {
        ...
        void* mapAddr;
#ifdef _WIN32
        mapAddr = malloc(dexOffset + dexLength);
        lseek(fd, 0, SEEK_SET);
		int cbRead = 0;
		while (cbRead < dexOffset + dexLength) {
			cbRead += read(fd, (char*)mapAddr + cbRead, dexOffset + dexLength - cbRead);
		}
#else
...
#ifdef FASTIVA
        ALOGE("Unable to create ClassLookup");
#endif
        goto bail;
	}
...
#ifdef FASTIVA
        ALOGE("Fail to CreateInlineSubsTable");
#endif
        goto bail;
	}
...
static bool loadAllClasses(DvmDex* pDvmDex)
{
     ...
     */
#ifndef FASTIVA
	// It is already processed in dvmClassStartup.
    ...
    for (idx = 0; idx < count; idx++) {
        ...
        if (newClass == NULL) {
            ALOGE("DexOpt: failed loading '%s'", classDescriptor);
            dvmClearOptException(dvmThreadSelf());
#ifdef _DEBUG
			while (false && newClass == NULL) {
				fastiva_debug_break("failed loading", true);
				newClass = dvmFindSystemClassNoInit(classDescriptor);
				dvmClearOptException(dvmThreadSelf());
			}
#endif
             ...
             */
#if 0 // def FASTIVA
			if (d2f_isFastivaClass(newClass) == 0) {
#endif
            ALOGE("DexOpt: '%s' has an earlier definition; blocking out",
                classDescriptor);
    ...
    const DexClassDef* pClassDef, bool doVerify, bool doOpt)
{
...
#ifdef FASTIVA
    if (clazz->pDvmDex == NULL || clazz->pDvmDex->pDexFile != pDexFile) {
#else
    if (clazz->pDvmDex->pDexFile != pDexFile) {
        ...
        ALOGD("DexOpt: not verifying/optimizing '%s': multiple definitions",
            clazz->descriptor == 0 ? "unknown clazz" : clazz->descriptor);
#else  // is Dalvk Bug ?? it make seg fault (0 == clazz->descriptor)
        ALOGD("DexOpt: not verifying/optimizing '%s': multiple definitions",
            clazz->descriptor);
#endif
        return;
