...
#define FASTIVA_OPT_INVOKE_INTERFACE 1
/*
...
rewrite_static_field:
        ...
        if (!matched && !essentialOnly) {
            switch (opc) {
#if FASTIVA_OPT_INVOKE_INTERFACE
			case OP_INVOKE_INTERFACE:
				{
					...
					if (baseMethod != NULL && baseMethod->clazz == gDvm.classJavaLangObject) {
						dvmUpdateCodeUnit(method, insns, (u2)((insns[0] & ~0xFF) | OP_INVOKE_VIRTUAL));
					}
					else {
						break;
					}
			...
			case OP_INVOKE_INTERFACE_RANGE:
				{
					...
					baseMethod = dvmOptResolveMethod(clazz, methodIdx, METHOD_VIRTUAL, NULL);
					if (baseMethod != NULL && baseMethod->clazz == gDvm.classJavaLangObject) {
						dvmUpdateCodeUnit(method, insns, (u2)((insns[0] & ~0xFF) | OP_INVOKE_VIRTUAL_RANGE));
					}
					else {
						break;
					}
...
#ifdef FASTIVA
		if ((resMethod->accessFlags & ACC_VIRTUAL$) != 0) {
			resMethod = resClass->vtable[resMethod->methodIndex];
		}
#endif
...
#ifdef FASTIVA
	// inhibit call (final native) Object.wait via invokeVirual.
	while ((baseMethod->accessFlags & ACC_FINAL$) != 0) {
		if ((baseMethod->accessFlags & ACC_VIRTUAL$) != 0) {
			break;
		}
		if (newOpc == OP_INVOKE_VIRTUAL_QUICK) {
			newOpc = OP_INVOKE_DIRECT;
		}
		else if (newOpc == OP_INVOKE_VIRTUAL_QUICK_RANGE) {
			newOpc = OP_INVOKE_DIRECT_RANGE;
		}
		else {
			break;
		}
...
#if FASTIVA_OPT_INVOKE_INTERFACE
		if (resMethod == NULL) {
            resMethod = dvmFindVirtualMethod(gDvm.classJavaLangObject,
                methodName, &proto);
		}
#endif
        if (resMethod == NULL) {
...
#if FASTIVA_OPT_INVOKE_INTERFACE
        if (!dvmIsAbstractMethod(resMethod) && resMethod->clazz != gDvm.classJavaLangObject) {
#else
...
#ifdef FASTIVA
    if ((insns[0] & 0xff) == OP_INVOKE_DIRECT 
	&&  (method->accessFlags & ACC_OVERRIDE_RET$) != 0) {
		ALOGD("OPT-Fastiva: direct call to bridge method %s.%s", method->clazz->descriptor, method->name);
...
#ifdef FASTIVA
    if ((insns[0] & 0xff) == OP_INVOKE_DIRECT_RANGE 
	&&  (method->accessFlags & ACC_OVERRIDE_RET$) != 0) {
		ALOGD("OPT-Fastiva: direct call to bridge method %s.%s", method->clazz->descriptor, method->name);
		rewriteVirtualInvoke(method, insns, OP_INVOKE_VIRTUAL_QUICK_RANGE);
	}
#endif
    return false;
}
