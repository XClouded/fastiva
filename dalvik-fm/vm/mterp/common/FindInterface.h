...
#ifdef FASTIVA
	// method = callerMethod;
    Method* absMethod = dvmDexGetResolvedMethod(methodClassDex, methodIdx);
    if (absMethod == NULL) {
		Method* d2f_resolveImplementedInterfaceMethod(ClassObject* thisClass, const Method* method, int methodIdx);
        absMethod = d2f_resolveImplementedInterfaceMethod(thisClass, method, methodIdx);
		if (absMethod == NULL) {
			assert(dvmCheckException(dvmThreadSelf()));
			return NULL;
		}
    }
	assert(thisClass->obj.itables$ != NULL);
	const int* indexTable = thisClass->obj.itables$[absMethod->clazz->ifc.itableId$];
	assert(indexTable != NULL);
	int slot = indexTable[absMethod->methodIndex];
	Method* m = thisClass->vtable[slot];
	assert(m != NULL);
	return m;
#else
...
#else
	int result;
    ATOMIC_CACHE_LOOKUP(methodClassDex->pInterfaceCache,
                DEX_INTERFACE_CACHE_SIZE, thisClass, methodIdx, result);
