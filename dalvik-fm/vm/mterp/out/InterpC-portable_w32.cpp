...
#define GOTO_TARGET(_target, ...)		int _target() {
...
#ifdef _WIN32
struct MethodFrame;
typedef int (MethodFrame::*OPFN)();
#endif

# define H(_op)					&&op_##_op ï¿½ï¿½ï¿?ï¿½ï¿½ ï¿½ï¿½.
# define HANDLE_OPCODE(_op)		int op_##_op() {
# define FINISH(_offset)		return _offset
# define FINISH_BKPT(_opcode) {                                             \
        OPFN addr;															\
        *(int*)&addr = (int)handlerTable[_opcode];                          \
		return (this->*addr)();													\
    }
...
#define GOTO_exceptionThrown() return exceptionThrown();

#define GOTO_returnFromMethod() return returnFromMethod();
...
#define GOTO_invokeMethod(_methodCallRange, _methodToCall, _vsrc1, _vdst) return invokeMethod();

#define GOTO_bail() return -100000;// goto bail;
...
struct MethodFrame {
	...
	static void* handlerTable[kNumPackedOpcodes];

void dvmInterpretPortable__start(Thread* self)
{
#if defined(EASY_GDB)
    debugSaveArea = SAVEAREA_FROM_FP(self->interpSave.curFrame);
#endif
	this->self = self;
...
#if 0 //def FASTIVA
	void* save_sp = &self->interpSave.bailPtr;
	_asm push eax;
	_asm mov save_sp, eax;
	_asm mov esp, [eax];
	_asm pop eax;
#endif
    /*
...
#endif
    int _offset = 0;                  /* fetch and execute first instruction */
	while (_offset > -100000) {
        ADJUST_PC(_offset);
        inst = FETCH(0);
        ...
        OPFN addr;
		*(int*)&addr = (int)handlerTable[INST_INST(inst)];
		_offset = (this->*addr)();
    }
...
GOTO_TARGET(invokeVirtualQuick, bool methodCallRange)
    {
         ...
         * method.
         */
		if (0 && ref >= (unsigned int) thisPtr->clazz->vtableCount) {
...
GOTO_TARGET(invokeMethod, bool methodCallRange, const Method* _methodToCall,
    u2 count, u2 regs)
    {
        ...
        int i;
#if 0
		if (d2f_isFastivaMethod(methodToCall)) {
			int* args = (int*)alloca(vsrc1 * 4);
			if (methodCallRange) {
				...
				memcpy(args, fp, vsrc1 * 4);
				typedef jlonglong (*fastiva_func)();
				retval.j = ((fastiva_func)methodToCall->fastivaMethod)();
				FINISH(3);
			} else {
				u4 count = vsrc1 >> 4;
				...
				typedef jlonglong (*fastiva_func5)(int, int, int, int, int);
				typedef jlonglong (*fastiva_func4)(int, int, int, int);
				typedef jlonglong (*fastiva_func3)(int, int, int);
				typedef jlonglong (*fastiva_func2)(int, int);
				typedef jlonglong (*fastiva_func1)(int);
				const void* func = methodToCall->fastivaMethod;
				switch (count) {
				case 5:
					retval.j = ((fastiva_func5)func)(GET_REGISTER(vdst & 0x0f), GET_REGISTER((vdst & 0x00f0) >> 4), GET_REGISTER((vdst & 0x0f00) >> 8), GET_REGISTER(vdst >> 12), GET_REGISTER(vsrc1 & 0x0f));
					break;
				case 4:
					retval.j = ((fastiva_func4)func)(GET_REGISTER(vdst & 0x0f), GET_REGISTER((vdst & 0x00f0) >> 4), GET_REGISTER((vdst & 0x0f00) >> 8), GET_REGISTER(vdst >> 12));
					break;
				case 3:
					retval.j = ((fastiva_func3)func)(GET_REGISTER(vdst & 0x0f), GET_REGISTER((vdst & 0x00f0) >> 4), GET_REGISTER((vdst & 0x0f00) >> 8));
					break;
				case 2:
					retval.j = ((fastiva_func2)func)(GET_REGISTER(vdst & 0x0f), GET_REGISTER((vdst & 0x00f0) >> 4));
					break;
				case 1:
					retval.j = ((fastiva_func1)func)(GET_REGISTER(vdst & 0x0f));
					break;
				default:
					;
				}
...
void* MethodFrame::handlerTable[kNumPackedOpcodes];

void fastiva_initInterpreterOperationTable() {

	int idxHandler = 0;
	#undef H2
	#define H2(_op) {									\
		OPFN fn = &MethodFrame::op_##_op;					\
		MethodFrame::handlerTable[idxHandler++] = *(void**)&fn;		\
	}
	DEFINE_GOTO_TABLE_w32();
}

void dvmInterpretPortable(Thread* self) {
	MethodFrame frame;
	frame.dvmInterpretPortable__start(self);
}