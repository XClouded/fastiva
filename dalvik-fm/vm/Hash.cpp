...
{
#ifdef FASTIVA
	HashTable* fastiva_dvmHashTableCreate(size_t initialSize, HashFreeFunc freeFunc, bool recursive);
	return fastiva_dvmHashTableCreate(initialSize, freeFunc, false);
}
...
HashTable* fastiva_dvmHashTableCreate(size_t initialSize, HashFreeFunc freeFunc, bool recursive)
{
    ...
    dvmInitMutex(&pHashTable->lock);
		...
		assert(res == 0);
		res = pthread_mutex_init(&pHashTable->lock, &attr);
		assert(res == 0);
