 ...
 */
int dvmImplements(const ClassObject* clazz, const ClassObject* _interface)
{
    int i;

    assert(dvmIsInterfaceClass(_interface));
    ...
    for (i = 0; i < clazz->iftableCount; i++) {
        if (clazz->iftable[i].clazz == _interface)
            return 1;
    }
...
#else
	int result; // zee
    ATOMIC_CACHE_LOOKUP(gDvm.instanceofCache,
                INSTANCEOF_CACHE_SIZE, instance, clazz, result);
	return result;
#endif
