...
extern void* fastiva_getFieldOffset(u1* ptr, int offset);
#endif
...
INLINE void dvmSetObjectArrayElement(const ArrayObject* obj, int index,
                                     Object* val) {
#ifdef FASTIVA
	FASTIVA_ASSERT(index >= 0 && index <= (int)obj->length);
#endif
#ifdef FASTIVA_GC_DEBUG
	fastiva.ensureArray(obj, sizeof(Object*));
#endif
    ((Object **)(void *)(obj)->contents)[index] = val;
    if (val != NULL) {
        dvmWriteBarrierArray(obj, index, index + 1);
#ifdef FASTIVA_GC_DEBUG
		fastiva_getFieldOffset((u1*)val, 4);
#endif
    }
}
 ...
 */
#ifdef FASTIVA_GC_DEBUG
#define BYTE_OFFSET(_ptr, _offset)  fastiva_getFieldOffset((u1*)(_ptr), _offset)
#else
