...
#ifdef FASTIVA
    for (int idx = 0; i < clazz->iftableCount; i++) {
        ClassObject* iface = clazz->iftable[idx++].clazz;
#else
    ...
    bool findVirtual, bool isHier, const char* name, const char* descriptor)
{
     ...
     */
#ifndef __GNUC__
    char* buffer = (char*)alloca(argCount + (returnType - descriptor) - 2);
    const char** argTypes = (const char**)alloca(sizeof(char*) * argCount);
#else
...
#ifdef FASTIVA
	const ClassObject* targetClass = clazz;
	bool isInterface = dvmIsInterfaceClass(clazz);
#endif
    while (clazz != NULL) {
        Method* methods;
        size_t methodCount;
        ...
        for (i = 0; i < methodCount; i++) {
            ...
            if (compareMethodHelper(method, name, returnType, argCount,
                            argTypes) == 0) {
#ifdef FASTIVA
		        if (!isInterface && (method->accessFlags & ACC_VIRTUAL$) != 0) {
					int mIndex = method->methodIndex;
					method = targetClass->vtable[mIndex];
					if (!isHier) {
						const ClassObject* superClass = targetClass->super;

						if (superClass != NULL
						&&  mIndex < superClass->vtableCount
						&&  method == superClass->vtable[mIndex]) {
							method = NULL;
						}
					}
				}
...
#ifdef FASTIVA
					if (!dvmIsInterfaceClass(targetClass) && (method->accessFlags & ACC_VIRTUAL$) != 0) {
						int mIndex = method->methodIndex;
						method = targetClass->vtable[mIndex];
						if (!isHier) {
							const ClassObject* superClass = targetClass->super;
...
#ifdef FASTIVA
    if (dvmIsDirectMethod(meth) || dvmIsFinalMethod(meth)) {
#else
...
#ifdef FASTIVA
        const int* methodTable = clazz->obj.itables$[meth->clazz->ifc.itableId$];
		methodIndex = methodTable[meth->methodIndex];
#else
        int i;
