    ...
    size_t elemWidth, int allocFlags)
{
    ...
    ArrayObject* newArray = (ArrayObject*)fastiva_dvmMalloc(totalSize, allocFlags, arrayClass);
#else
...
#else
		//d2f_initObject(newArray, arrayClass);
#endif
        newArray->length = length;
...
#ifndef __GNUC__
    char* className = (char*)alloca(nameLen + 2);
#else
...
static ClassObject* createArrayClass(const char* descriptor, Object* loader)
{
     ...
     */
#ifdef FASTIVA // _SUSPEND_BY_SIGNAL
    newClass = (ClassObject*) fastiva_dvmMalloc(sizeof(fastiva_Class), ALLOC_NON_MOVING, gDvm.classJavaLangClass);
#else
...
#ifdef FASTIVA
	if (arrayDim <= FASTIVA_MAX_ARRAY_DIMENSION && d2f_isFastivaClass(elementClass)) {
	    newClass->descriptor = elementClass->descriptor - arrayDim;
		assert(newClass->descriptor[0] == '[');
	}
	else {
...
#ifdef FASTIVA
	{
		extern void d2f_initArrayClass(ClassObject* newClass, ClassObject* elementClass);
		d2f_initArrayClass(newClass, elementClass);
	}
#else
...
#ifdef FASTIVA
	if (d2f_isFastivaClass(elementClass)) {
		// do nothing
	}
	else
#endif
