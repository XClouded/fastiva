...
{
#ifdef FASTIVA
    return sizeof(fastiva_Class) + sizeof(StaticField) * sfieldCount;
#else
...
#ifdef FASTIVA // _SUSPEND_BY_SIGNAL
    ClassObject* newClass = (ClassObject*) fastiva_dvmMalloc(sizeof(fastiva_Class), ALLOC_NON_MOVING, gDvm.classJavaLangClass);
#else
...
#ifndef FASTIVA // _SUSPEND_BY_SIGNAL
...
static bool createInitialClasses() {
    ...
    ClassObject* clazz;
	{
		...
		clazz = d2f_initClassOfJavaLangClass();
	}
#else
...
#ifdef FASTIVA
	{
		extern void fastiva_initKernel();
		fastiva_initKernel();
	}
#endif
    /*
...
#ifdef FASTIVA
	{
		extern void fastiva_initRuntime();
		fastiva_initRuntime();
	}
	// For framework/*.odex generation.
    if (!dvmFindRequiredClassesAndMembers()) {
        return false;//"dvmFindRequiredClassesAndMembers failed";
    }
#endif
    /*
...
static bool prepareCpe(ClassPathEntry* cpe, bool isBootstrap)
{
    ...
    if ((strcmp(suffix, "jar") == 0) || (strcmp(suffix, "zip") == 0) ||
		...
		char odex[1024];
		if (isBootstrap) {
			strcpy(odex, cpe->fileName);
			strcpy(odex + strlen(odex) - 3, "odex");
			dexName = odex;
		}
        if (dvmJarFileOpen(cpe->fileName, dexName, &pJarFile, isBootstrap) == 0) {
#else
...
#ifdef _WIN32
	char* urlBuf = (char*)alloca(strlen(name) + strlen(cpe->fileName) + kUrlOverhead +1);
#else
...
#if 0 // def _DEBUG
	if (!strcmp(clazz->descriptor, "Lorg/keyczar/KeyMetadata;")) {
		fastiva_debug_break(clazz->descriptor, true);
	}
#endif
...
#ifdef FASTIVA
        TRY${
	    clazz = ((java_lang_ClassLoader_p)loader)->loadClass_((java_lang_String_p)nameObj);
        }
        CATCH_ANY$ {
    ...
    DvmDex* pDvmDex)
{
    ...
    if (clazz == NULL) {
        ...
        if (pDvmDex == NULL) {
#ifdef FASTIVA
			clazz = NULL;// @TODO later (ClassObject*)fm::getRawClass(descriptor);
			assert(clazz == NULL);
			if (clazz != NULL) {
				return clazz;
			}
            ...
            //assert(loader == NULL || gDvm.bootClassPathOptExtra != NULL);     /* shouldn't be here otherwise */
#else
...
#ifdef _DEBUG
			//ALOGD("noClassDefFoundError: %s",  descriptor);
			//assert(strcmp(descriptor, "Lcom/android/bluetooth/btservice/AdapterService;") != 0);
#endif

            if (gDvm.noClassDefFoundErrorObj != NULL) {
                /* usual case -- use prefabricated object */
#ifdef FASTIVA
		        d2f_setPendingException(self, gDvm.noClassDefFoundErrorObj, true);
#else
    ...
    const u1* pEncodedData, Object* classLoader)
{
...
#ifdef FASTIVA // _SUSPEND_BY_SIGNAL
        newClass = (ClassObject*) fastiva_dvmMalloc(size, ALLOC_NON_MOVING, gDvm.classJavaLangClass);
#else
    ...
    if (newClass == NULL)
        return NULL;
#ifdef FASTIVA
    newClass->sfields = (StaticField*)((char*)newClass + sizeof(fastiva_Class));
#endif
#ifndef FASTIVA // _SUSPEND_BY_SIGNAL
    DVM_OBJECT_INIT(newClass, gDvm.classJavaLangClass);
#endif
...
#ifdef FASTIVA
	assert(!d2f_isFastivaClass(newClass));
#endif
...
#ifdef FASTIVA
        newClass->iftable = (InterfaceEntry*) dvmLinearAlloc(classLoader,
                newClass->interfaceCount * sizeof(InterfaceEntry));

        for (i = 0; i < newClass->interfaceCount; i++) {
            const DexTypeItem* pType = dexGetTypeItem(pInterfacesList, i);
            newClass->iftable[i].methodIndexArray = (int*)(u4) pType->typeIdx;
        }
#else
...
#ifdef FASTIVA
			newClass->virtualMethods[i].accessFlags |= ACC_VIRTUAL$;
#endif
            if (classMapData != NULL) {
                const RegisterMap* pMap = dvmRegisterMapGetNext(&classMapData);
                if (dvmRegisterMapGetFormat(pMap) != kRegMapFormatNone) {
...
#ifdef FASTIVA
	bool isFastivaClass	= d2f_isFastivaClass(clazz);
#define dvmLinearReadWrite_EX(a, b) if (!isFastivaClass) dvmLinearReadWrite(a, b)
#define dvmLinearFree_EX(a, b) if (!isFastivaClass) dvmLinearFree(a, b)
#define dvmLinearReadOnly_EX(a, b) if (!isFastivaClass) dvmLinearReadOnly(a, b)
#endif
    ...
    if (clazz->directMethods != NULL) {
        ...
        dvmLinearReadWrite_EX(clazz->classLoader, directMethods);
        for (i = 0; i < directMethodCount; i++) {
            freeMethodInnards(&directMethods[i]);
        }
        dvmLinearReadOnly_EX(clazz->classLoader, directMethods);
        dvmLinearFree_EX(clazz->classLoader, directMethods);
    }
    if (clazz->virtualMethods != NULL) {
        ...
        dvmLinearReadWrite_EX(clazz->classLoader, virtualMethods);
        for (i = 0; i < virtualMethodCount; i++) {
            freeMethodInnards(&virtualMethods[i]);
        }
        dvmLinearReadOnly_EX(clazz->classLoader, virtualMethods);
        dvmLinearFree_EX(clazz->classLoader, virtualMethods);
    }
...
#ifdef FASTIVA
	if (!isFastivaClass && clazz->arrayDim == 0) {
#endif
...
#ifdef FASTIVA
	if (d2f_isFastivaMethod(meth) != 0) {
		pMap = meth->registerMap = NULL;
	}
#endif
...
#ifdef FASTIVA
		meth->jniArgInfo = d2f_computeInterpreterArgInfo(meth);
#endif
    } else {
        /*
...
#ifdef FASTIVA
			FASTIVA_ASSERT(d2f_isFastivaMethod(meth) == 0);
#endif
...
{
#ifdef FASTIVA
	int fastiva_dvmComputeJniArgInfo(const char* sig);
    const char* sig = dexProtoGetShorty(proto);
	return fastiva_dvmComputeJniArgInfo(sig);
}
...
int fastiva_dvmComputeJniArgInfo(const char* sig) {
    ...
    hints = fastiva_dvmPlatformInvokeHints(sig);
#else
 ...
 */
#ifdef FASTIVA
bool fastiva_dvmPrecacheReferenceOffsets(ClassObject* clazz) {
	return precacheReferenceOffsets(clazz);
}

void fastiva_dvmComputeRefOffsets(ClassObject* clazz) {
...
bool dvmLinkClass(ClassObject* clazz)
{
    ...
    int i;
#ifdef FASTIVA
	assert(!d2f_isFastivaClass(clazz));
	if (false) {
		fm::getRawClass(clazz);
		ALOGD("link Fastiva class %s", clazz->descriptor);
		return true;
	}
#endif
        ...
        if (clazz->interfaceCount > 0) {
            ...
            for (i = 0; i < clazz->interfaceCount; i++) {
#ifdef FASTIVA
				uint interfaceIdx = (uint)clazz->iftable[i].methodIndexArray;
                assert(interfaceIdx != kDexNoIndex);
				clazz->iftable[i].clazz =
					dvmResolveClass(clazz, interfaceIdx, false);
                if (clazz->iftable[i].clazz == NULL) {
#else
				int interfaceIdx = interfaceIdxArray[i];
                assert(interfaceIdx != kDexNoIndex);
                clazz->interfaces[i] =
                    dvmResolveClass(clazz, interfaceIdx, false);
                if (clazz->interfaces[i] == NULL) {
                    ...
                    classDescriptor =
                        dexStringByTypeIdx(pDexFile, interfaceIdx);
                    if (gDvm.optimizing) {
                        /* happens with "external" libs */
...
#ifdef FASTIVA
                if (!dvmCheckClassAccess(clazz, clazz->iftable[i].clazz)) {
                    ALOGW("Interface '%s' is not accessible to '%s'",
                         clazz->iftable[i].clazz->descriptor, clazz->descriptor);
...
#ifdef FASTIVA
	FASTIVA_ASSERT(maxCount < 1024);
	void d2f_initFastivaVirtualSlot(Method* localMeth, int si);
    clazz->obj.vtable$ = (int*) dvmLinearAlloc(clazz->classLoader,
                        sizeof(Method*) * maxCount);
#endif
    if (clazz->super != NULL) {
        int actualCount;
...
#ifdef FASTIVA
	    memcpy((void*)clazz->obj.vtable$, clazz->super->obj.vtable$,
            sizeof(*(clazz->obj.vtable$)) * clazz->super->vtableCount);
#endif
         ...
         */
#ifdef FASTIVA_OPT_FINAL
        for (i < clazz->virtualMethodCount; --i >= 0; ) {
#else
...
#ifdef FASTIVA
					d2f_initFastivaVirtualSlot(localMeth, si);
#endif
...
#ifdef FASTIVA
				// @zee Non-overriding function can be called by Interface impilementation.
				d2f_initFastivaVirtualSlot(localMeth, actualCount);
#ifdef FASTIVA_OPT_FINAL
				if ((localMeth->accessFlags & ACC_FINAL) != 0) {
					_asm int 3;
					// !!! virtualMethods & directMethods are allocated seperately.
					localMeth->accessFlags &= ~ACC_VIRTUAL$;
					clazz->directMethods--;
					clazz->directMetodCount++;
					clazz->virtualMethodCount--;
					if (i != clazz->virtualMethodCount) {
						Method tmp = *this->directMethods;
						*this->directMethods = *localMeth;
						*localMeth = tmp;
					}
					continue;
				}
...
static bool createIftable(ClassObject* clazz)
{
...
#ifdef FASTIVA
	int ownIfCount = clazz->interfaceCount;
    for (int i = 0; i < clazz->interfaceCount; i++)
        ownIfCount += clazz->iftable[i].clazz->iftableCount;
	ifCount += ownIfCount;
#else
     ...
     */
#ifdef FASTIVA
	InterfaceEntry* rawIFTable = clazz->iftable;
#endif
    ...
    for (int i = 0; i < clazz->interfaceCount; i++) {
#ifdef FASTIVA
        ClassObject* interf = rawIFTable[i].clazz;
#else
...
#ifdef FASTIVA
    if (superIfCount != 0) {
        memcpy(clazz->iftable + idx, clazz->super->iftable,
            sizeof(InterfaceEntry) * superIfCount);
    }
	ownIfCount = idx;
	ifCount = ownIfCount + superIfCount;
	dvmLinearFree(clazz->classLoader, rawIFTable);
#else
    ifCount = idx;
#endif
     ...
     */
#ifdef FASTIVA
    if (dvmIsInterfaceClass(clazz) || ownIfCount == 0) {
#else
    ...
    for (int i = superIfCount; i < ifCount; i++) {
#endif
        ClassObject* _interface;
        int methIdx;

        clazz->iftable[i].methodIndexArray = clazz->ifviPool + poolOffset;
        _interface = clazz->iftable[i].clazz;
        poolOffset += _interface->virtualMethodCount;    // end here
         ...
         */
        for (methIdx = 0; methIdx < _interface->virtualMethodCount; methIdx++) {
            Method* imeth = &_interface->virtualMethods[methIdx];
            int j;
            ...
            if (j < 0) {
#ifdef FASTIVA_NO_SCAN_SUPER_INTEFACE_METHODS
				// @zee
				// not-implemented interface method should be automatically registered as abstract method.
				// The abstact method should be generated by Jpp compiler. so no code reach here!!
				FASTIVA_DBREAK();
#endif
                IF_ALOGV() {
                    char* desc =
    ...
    if (mirandaCount != 0) {
        ...
        for (int i = 0; i < mirandaCount; i++, meth++) {
            dvmLinearReadWrite(clazz->classLoader, clazz->virtualMethods);
#ifdef FASTIVA
			Method* inherited = dvmFindDirectMethodHier(clazz->super, mirandaList[i]->name, &mirandaList[i]->prototype);
			if (inherited != NULL && (inherited->accessFlags & ACC_PRIVATE) == 0) {
	            cloneMethod(meth, inherited);
			}
			else 
#endif
...
#ifdef FASTIVA
			meth->nativeFunc = (DalvikBridgeFunc) fastiva_Runtime::throwAbstractMethodError;
#else
            meth->nativeFunc = (DalvikBridgeFunc) dvmAbstractMethodStub;
#endif
        }
...
static bool computeFieldOffsets(ClassObject* clazz)
{
    ...
    if (i != clazz->ifieldCount && (fieldOffset & 0x04) != 0) {
        ...
        if (c != 'J' && c != 'D') {
            ...
            assert(c != '[' && c != 'L');
            pField->byteOffset = fieldOffset;
/// @Zee 32bit slot used for type below 4byte size. ��.��
            fieldOffset += sizeof(u4);
            i++;
            LOGVV("  --- offset2 '%s'=%d",
...
#ifdef _DEBUG
	if (result1 == NULL) {
		ALOGE("@@@@@ NOT FOUND %s in %s cl=%p", descriptor, clazz1->descriptor, clazz1->classLoader);
		assert(result1 != NULL);
	}
#endif
...
static bool checkMethodDescriptorClasses(const Method* meth,
    const ClassObject* clazz1, const ClassObject* clazz2)
{
    // @Zee. DexFiles can have same class name int in multi-classloader
...
#ifdef FASTIVA
		ALOGD("+++ late optimize on %s (pv=%d)",
            clazz->descriptor, IS_CLASS_FLAG_SET(clazz, CLASS_ISPREVERIFIED));
		// @Zee must fully optimized in Fastiva mode. (for final/virtual detectection)
    ...
    const u2* insns)
{
    ...
    if (insns != NULL) {
        ...
        method->insns = insns;
#ifdef FASTIVA
		if (d2f_isFastivaMethod(method) == 0) 
#endif
...
void dvmSetRegisterMap(Method* method, const RegisterMap* pMap)
{
    ClassObject* clazz = method->clazz;
#ifdef FASTIVA
	FASTIVA_ASSERT(d2f_isFastivaMethod(method) == 0);
#endif
...
#ifdef FASTIVA
	Object* loader = java_lang_ClassLoader_C$::getRawStatic$()->getSystemClassLoader_();
#else
	JValue result;
