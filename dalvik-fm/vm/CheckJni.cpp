...
static void abortMaybe() {
    if (!gDvmJni.warnOnly) {
#ifdef FASTIVA
		fastiva_debug_break("CheckJNI fail", true);
#else
...
#endif
#ifdef FASTIVA
		oldStatus = dvmChangeStatus(mSelf, THREAD_RUNNING);
#else
...
static jint Check_RegisterFastNatives(JNIEnv* env, jclass c, const JNINativeMethod* methods, jint nMethod) {
	return Check_RegisterNatives(env, c, methods, nMethod);
}

void Check_PushExceptionHandler(fastiva_ExceptionContext* exContext) {
	void fastiva_PushExceptionHandler(fastiva_ExceptionContext* exContext);
	fastiva_PushExceptionHandler(exContext);
}
void Check_RemoveExceptionHandler(fastiva_ExceptionContext* exContext) {
	void fastiva_RemoveExceptionHandler(fastiva_ExceptionContext* exContext);
	fastiva_RemoveExceptionHandler(exContext);
}

static void Check_ReleaseArray(jobject) {
	void fastiva_ReleaseArray(jobject);
	fastiva_ReleaseArray(NULL);
}
...
#if 0 //def FASTIVA
    Check_RegisterFastNatives,
    Check_ReleaseArray,
    (FASTIVA_EXCEPTION_CONTEXT_HANDLER_FUNC)Check_PushExceptionHandler,
    (FASTIVA_EXCEPTION_CONTEXT_HANDLER_FUNC)Check_RemoveExceptionHandler,
#endif
};
