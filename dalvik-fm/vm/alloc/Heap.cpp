...
static void throwOOME()
{
    ...
    if ((self = dvmThreadSelf()) != NULL) {
        ...
        if (dvmIsOnThreadList(self) && !self->throwingOOME) {
             ...
             */
#ifdef FASTIVA
            d2f_setPendingException(self, gDvm.outOfMemoryObj, true);
#else
...
void* dvmMalloc(size_t size, int flags)
{
	...
	return fastiva_dvmMalloc(size, flags, NULL);
}
...
bool fastiva_dvmIsValidDataObject(const Object* obj)
{
    if (obj != NULL) {
		if (((uintptr_t)obj & (8-1)) == 0) {
			bool fastiva_dvmHeapSourceContains(const void *ptr);
			return fastiva_dvmHeapSourceContains(obj);
		}
    }
	return false;
}
...
int	gc_start_threadId;
pthread_t gc_start_thread;
#endif
/*
