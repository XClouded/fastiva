...
#include <java/lang/ref/ReferenceQueue.inl>

#define FASTIVA_GC
//#define FASTIVA_GC_DEBUG
void fastiva_dvmMarkImmuneObjects(const char *immuneLimit, BitmapCallback *callback,
                       void *arg);
...
static bool isMarked(const Object *obj, const GcMarkContext *ctx)
{
	...
	return dvmIsClassObject(obj) || dvmHeapBitmapIsObjectBitSet(ctx->bitmap, obj);
#else
                              ...
                              bool checkFinger)
{
    ...
    if (dvmIsClassObject(obj)) {
		if (d2f_isFastivaClass((ClassObject*)obj)) {
		return;
	}
	}
#endif
...
void dvmHeapReMarkRootSet()
{
	...
	void fastiva_dvmScanStatic(Visitor *callback, void *arg);
	fastiva_dvmScanStatic((Visitor*)scanClassObject, ctx);
...
#ifdef FASTIVA
        markObject((const Object *)clazz->iftable[i].clazz, ctx);
#else
...
static void scanClassObject(const Object *obj, GcMarkContext *ctx)
{
	...
	if (d2f_isFastivaClass(asClass) == 0) { // || asClass->m_pContext$->m_pClass0 != asClass) {
		setAndReturnMarkBit(ctx, asClass);
	}
    markObject((const Object *)asClass->classLoader, ctx);
	markObject(((fastiva_Class_p)asClass)->get__name(), ctx);
#else
...
#ifdef FASTIVA_GC_DEBUG
    if (obj == debug_obj) {
		ALOGD("marking debug_obj as a array");
}
#endif
...
#ifdef FASTIVA_GC_DEBUG
    if (obj == debug_obj && contents[i] != NULL) {
				ALOGD("marking debug_obj as a array item %s", contents[i]->clazz->descriptor);
}
#endif
...
void dvmHeapScanMarkedObjects(void)
{
	...
	if (ctx->bitmap->max > ctx->bitmap->base) 
#endif
...
void dvmSetFinalizable(Object *obj)
{
    assert(obj != NULL);
#ifdef FASTIVA
	java_lang_ref_FinalizerReference_C$::getRawStatic$()->add_(obj);
#else
...
void dvmEnqueueClearedReferences(Object **cleared)
{
    ...
    if (*cleared != NULL) {
#ifdef FASTIVA
		java_lang_ref_ReferenceQueue_C$::getRawStatic$()->add_((java_lang_ref_Reference_p)*cleared);
#else
        Thread *self = dvmThreadSelf();
        assert(self != NULL);
...
{
#ifdef FASTIVA_PRELOAD_STATIC_INSTANCE
    return !dvmHeapBitmapIsObjectBitSet(gDvm.gcHeap->markContext.bitmap, (Object *)obj);
#else
