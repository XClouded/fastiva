...
Object* dvmAllocObject(ClassObject* clazz, int flags)
{
	...
	if (kernelData.vmInitialized) {
		Thread* self = dvmThreadSelf();
		const fastiva_Class_p* ppClass = clazz->preimportedClasses;
		if (ppClass != NULL) {
			fastiva_Class_p pClass;
			while ((pClass = *ppClass) != NULL) {
				if (pClass->status < CLASS_INITIALIZING) {
					d2f_initStatic(self, pClass);
    ...
    newObj = (Object*)fastiva_dvmMalloc(clazz->objectSize, flags, clazz);
#else
...
#ifdef FASTIVA // _SUSPEND_BY_SIGNAL
    Object* copy = (Object*)fastiva_dvmMalloc(size, flags, clazz);
#else
...
#else
    //	d2f_initObject(copy, clazz);
	FASTIVA_ASSERT(*(int*)copy == *(int*)obj);
#endif
