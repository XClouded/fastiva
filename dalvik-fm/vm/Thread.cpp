...
#if DFASTIVA_TARGET_ANDROID_VERSION>=40400
 #include <cutils/trace.h>
#else
# define ATRACE_BEGIN(...) // IGNORE
# define ATRACE_END() // IGNORE
#endif
...
#if defined(FASTIVA) && defined(_DEBUG)
...
#ifdef FASTIVA
    if (dvmTryLockMutex(&gDvm.threadListLock) != 0) {
		if (self == NULL)       /* try to get it from TLS */
			self = dvmThreadSelf();

		if (self != NULL) {
			FASTIVA_SUSPEND_STACK_safe(self);
			...
			FASTIVA_RESUME_STACK_safe(self);
		} else {
			dvmLockMutex(&gDvm.threadListLock);
		}
	}
...
pid_t dvmGetSysThreadId()
{
	...
	return fastiva_getCurrentThreadID();
#endif
#ifdef HAVE_GETTID
    return gettid();
#else
...
static bool prepareThread(Thread* thread)
{
...
#ifdef FASTIVA
	{
		...
		thread->m_pNativeStackBottom = (void*)fastiva_sys_getCurrentStackBottom();
#if FASTIVA_SCAN_INTO_JNI_STACK
		assert(thread->status == THREAD_RUNNING || thread->status == THREAD_NATIVE
			|| thread->status == THREAD_INITIALIZING );
		assert(thread->m_pNativeStackPointer == NULL);
#else
		if (thread->status == THREAD_RUNNING) {
			thread->m_pNativeStackPointer = NULL;
		}
		else {
			thread->m_pNativeStackPointer = thread->m_pNativeStackBottom;
		}
#endif
	}
#endif
...
bool dvmCreateInterpThread(Object* threadObj, int reqStackSize)
{
     ...
     */
#ifdef FASTIVA
	FASTIVA_ASSERT(!dvmCheckException(self));
#endif
    dvmLockThreadList(self);
    ...
    if (dvmGetFieldObject(threadObj, gDvm.offJavaLangThread_vmThread) != NULL) {
		...
		fastiva_debug_break("thread has already been started", true);
#else
        dvmUnlockThreadList();
        dvmThrowIllegalThreadStateException(
            "thread has already been started");
...
#ifdef FASTIVA
	if (d2f_isFastivaMethod(run)) {
		typedef void (*RUN_FN)(void* self);
		dvmPushJNIFrame(self, run);
		TRY$ {
			((RUN_FN)run->fastivaMethod)(self->threadObj);
		}
		CATCH_ANY$ {
		}
...
void dvmDetachCurrentThread()
{
     ...
     */
    if (group != NULL) {
#ifdef FASTIVA //_NO_VIRTUAL_METHOD_IN_FINAL_CLASS
        Method* removeThread = gDvm.voffJavaLangThreadGroup_removeThread;
/*
	    ThreadStatus old_st = dvmChangeStatus(self, THREAD_RUNNING);
#ifdef _DEBUG
		ALOGE("Thread(%p:%d) st on detach: %d", self, self->threadId, self->status);
#endif
*/
#else
        Method* removeThread =
...
void dvmSuspendSelf(bool jdwpActivity)
{
	...
	void fastiva_dvmAddToSuspendCounts_unsafe(Thread* thread, int delta, int dbgDelta);
    fastiva_dvmAddToSuspendCounts_unsafe(self, 1, 1);
#else
...
#ifdef FASTIVA
    while (thread->status == THREAD_RUNNING || (!FASTIVA_IS_STACK_CLOSED(thread) && thread->m_pNativeStackBottom != 0)) {
#else
...
#ifdef FASTIVA // for suspend by signal
...
#ifdef FASTIVA
void fastiva_dvmFullSuspendCheck(Thread* self) {
...
{
#if defined(FASTIVA_SUSPEND_BY_SIGNAL) 
	return false;
#else
    assert(self != NULL);
    if (self->suspendCount == 0) {
...
#ifdef FASTIVA
bool fastiva_dvmCheckSuspendPending(Thread* self)
{
    assert(self != NULL);
    if (self->suspendCount == 0) {
        return false;
...
#ifdef FASTIVA
		self->m_pNativeStackPointer = NULL;
#endif
        if (self->suspendCount != 0) {
            fullSuspendCheck(self);
        }
...
#ifdef FASTIVA
	#if	!FASTIVA_SCAN_INTO_JNI_STACK
		assert (newStatus != THREAD_NATIVE || self->m_pNativeStackPointer != NULL || self->m_pNativeStackBottom == NULL);
	#endif
#endif
        /*
...
#define getpriority(type, tid)	0
    ...
    dvmPrintDebugMessage(target, "\n");
#if defined(FASTIVA) && defined(_DEBUG)
	Object* pRun = thread->threadObj;
	if (pRun != NULL){
		java_lang_Thread_p pCurrThread = (java_lang_Thread_p)pRun;
		java_lang_Runnable_p pRunnable = pCurrThread->get__target();
		if (pRunnable != NULL) {
			pRun = pRunnable;
		}
		ALOGE("Thread: %s", pRun->clazz->descriptor);
	}
#endif
...
bool fastiva_dvmRequestSuspendAllThread() {
	Thread* self = dvmThreadSelf();
	if (dvmTryLockMutex(&gDvm._threadSuspendLock) != 0) {
		return false;
	}
	dvmLockThreadList(self);
	bool locked = dvmTryLockMutex(&gDvm.threadSuspendCountLock) == 0;
    if (locked) {
		for (Thread* thread = gDvm.threadList; thread != NULL; thread = thread->next) {
			if (thread == self)
				continue;
