    ...
    Object* cause)
{
...
#ifdef _DEBUG
	{
		...
		check_break_for_exception(self, (java_lang_Throwable_p)exception);
	}
#endif
...
#ifdef _DEBUG
	if (catchAddr < 0) {
		ALOGD("Excation not catched in %s", exception->clazz->descriptor);
	}
	else {
		StackSaveArea* saveArea = SAVEAREA_FROM_FP(fp);
		ALOGD("Catched in %s.%s:%d", saveArea->method->clazz->descriptor, saveArea->method->name, catchAddr);
	}
#endif
    return catchAddr;
}
...
#ifdef FASTIVA
                if (!(saveArea->xtra.currentPc >= method->insns &&
...
#ifdef FASTIVA
	// 2014.1202 gms-2.apk 에서 Thread->getStackTrace() 함수를 이용해 로그를 찍는 부분 처리를 위한 것이다.
	// 최소 5개의 stackElement가 필요하다.
	size_t depth = stackDepth;
	if (stackDepth > 0 && depth < 5) {
		depth = 5;
	}
    ArrayObject* array = dvmAllocArrayByClass(klass, depth, ALLOC_DEFAULT);
#else
...
#ifdef FASTIVA
	  if (depth > stackDepth) {
		  java_lang_Object_ap list = (java_lang_Object_ap)array;
		  Object* last = list->get$(stackDepth - 1);
		  for (; stackDepth < depth; stackDepth ++) {
			  list->set$(stackDepth, last);
		  }
	  }
#endif
