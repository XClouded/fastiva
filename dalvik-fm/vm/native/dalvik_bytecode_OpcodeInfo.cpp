...
#ifdef FASTIVA
	jbool fastiva_Dalvik_dalvik_bytecode_OpcodeInfo_isInvoke(jint);
	*(jbool*)pResult = fastiva_Dalvik_dalvik_bytecode_OpcodeInfo_isInvoke(opcode);
}

jbool fastiva_Dalvik_dalvik_bytecode_OpcodeInfo_isInvoke(jint opcode) {
#endif
    int flags = dexGetFlagsFromOpcode((Opcode)opcode);
...
const DalvikNativeMethod dvm_dalvik_bytecode_OpcodeInfo[] = {
    { "isInvoke", "(I)Z", FASTIVA_INTERNAL_METHOD_PAIR(Dalvik_dalvik_bytecode_OpcodeInfo_isInvoke) },
    { NULL, NULL, NULL, NULL },
};
