...
static void setCommandLineDefaults()
{
     ...
     */
#ifdef _WIN32
    gDvm.heapStartingSize = 160 * 1024 * 1024;  // Spec says 16MB; too big for us.
    gDvm.heapMaximumSize = 160 * 1024 * 1024;  // Spec says 75% physical mem
#else
        ...
        bool ignoreUnrecognized, JNIEnv* pEnv)
{
...
#ifdef FASTIVA
	{
		extern void fastiva_initRuntime_ex();
		fastiva_initRuntime_ex();
	}
#endif
    /*
...
#ifdef FASTIVA
	{
		...
		fastiva_initPreImportedClasses();
	}
#endif
...
#ifdef _WIN32
	{
		extern int JNI_loadLibCore(JNIEnv* pEnv);
		JNI_loadLibCore(pEnv);

		//extern int registerJniHelp(JNIEnv*); 
	    //registerJniHelp(pEnv);
	}
#else
...
#ifdef FASTIVA
	FASTIVA_SET_NATIVE_STACK_POINTER(self, NULL);
#endif
    ...
    DexClassVerifyMode verifyMode, int dexoptFlags)
{
    ...
    setCommandLineDefaults();
#ifdef _DEBUG
	//ALOGE("# gDvm.classpath is '%s'", gDvm.classPathStr);
	//ALOGE("###### bootclasspath is '%s'", bootClassPath);
#endif
    free(gDvm.bootClassPathStr);
...
{
#ifdef FASTIVA
	fastiva_debug_break("abort", true);
#endif
