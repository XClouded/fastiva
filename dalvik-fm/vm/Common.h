...
#ifndef LOG_TAG
#ifdef FASTIVA
# define LOG_TAG "fastiva"
#else
//#error "No Fastiva defined"
...
#  define assert(x) ((x) ? ((void)0) : (ALOGE("ASSERT FAILED (%s:%d): %s", \
        __FILE__, __LINE__, #x), fastiva_debug_break(#x, true), (void)0) )
# else
#  define assert(x) \
