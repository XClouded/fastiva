...
bool IndirectRefTable::remove(u4 cookie, IndirectRef iref)
{
    ...
    assert(alloc_entries_ <= max_entries_);
    assert(segmentState.parts.numHoles >= prevState.parts.numHoles);
	volatile IRTSegmentState prevState2 = prevState;
    assert(prevState2.parts.numHoles >= prevState.parts.numHoles);

    IndirectRefKind kind = indirectRefKind(iref);
    u4 index;
#if FASTIVA_FAST_JNI
	if (kind != kIndirectKindWeakGlobal) {
        index = findObject(reinterpret_cast<Object*>(iref), bottomIndex, topIndex, table_);
        if ((int)index < 0) {
            return false;
        }
    }
