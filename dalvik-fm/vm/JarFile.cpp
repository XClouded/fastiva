...
int fastiva_openAlternateSuffix(const char *fileName, const char *suffix,
    int flags, char **pCachedName) {
		return openAlternateSuffix(fileName, suffix, flags, pCachedName);
}
#endif
/*
...
#ifdef FASTIVA
		if (FASTIVA_IS_PRECOMPILED(fileName)) {
        return DEX_CACHE_OK;
	}
	char* fastiva_hasCompiled(const char* sourceName, const char* name);
	char* complieldSoName = fastiva_hasCompiled(fileName, NULL);
	if (complieldSoName != NULL) {
		free((void*)complieldSoName);
        return DEX_CACHE_OK;
	}
#endif
    ...
    JarFile** ppJarFile, bool isBootstrap)
{
	...
	else if ((cachedName = fastiva_hasCompiled(fileName, NULL)) != NULL) {
		//char* endOfName = strrchr(cachedName, '.');
		//endOfName[0] = 0;
		free(cachedName);
        cachedName = dexOptGenerateCacheFileName(fileName,
                                kDexInJarName);
		ALOGD("%s dex is compiled.so", cachedName);
		goto already_compiled;
	}
	else 
#endif
    //fd = openAlternateSuffix(fileName, "odex", O_RDONLY, &cachedName);
    if ((fd = openAlternateSuffix(fileName, "odex", O_RDONLY, &cachedName)) >= 0) {
...
tryArchive:
        ...
        if (entry != NULL) {
			...
			fastiva_debug_break("No .so", true);
