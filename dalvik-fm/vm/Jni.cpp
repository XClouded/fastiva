...
#if defined WITH_JNI_STACK_CHECK && !FASTIVA_FAST_JNI
...
public:
    explicit ScopedJniThreadState(JNIEnv* env) {
		...
		if (FASTIVA_SCAN_INTO_JNI_STACK) {
			assert(mSelf->m_pNativeStackPointer == NULL);
		}
		else if (mSelf->m_pNativeStackBottom == NULL) {
			assert(mSelf->m_pNativeStackPointer == NULL);
			int fastiva_sys_getCurrentStackBottom();
			mSelf->m_pNativeStackBottom = (void*)this;//fastiva_sys_getCurrentStackBottom();
			this->oldSP = (void*)-1;
		}
		else {
			this->oldSP = mSelf->m_pNativeStackPointer;
#if FASTIVA_SCAN_INTO_JNI_STACK
			assert(this->oldSP == NULL);
#else
			assert(this->oldSP != NULL);
#endif
		}
		mOldStatus = dvmChangeStatus(mSelf, THREAD_RUNNING);
#else 
    ...
    ~ScopedJniThreadState() {
		...
		else if (this->oldSP == (void*)-1) {
			mSelf->m_pNativeStackBottom = NULL;
		}
		else {
			mSelf->m_pNativeStackPointer = this->oldSP;
		}
		dvmChangeStatus(mSelf, mOldStatus);
#else 
...
Object* dvmDecodeIndirectRef(Thread* self, jobject jobj) {
    ...
    case kIndirectKindLocal:
...
#endif
        {
			...
			fastiva_getFieldOffset((u1*)result, 4);
#endif
            return result;
        }
    case kIndirectKindGlobal:
#if FASTIVA_FAST_JNI
        return reinterpret_cast<Object*>((jint)jobj & ~3);
#endif
        {
...
#if FASTIVA_SCAN_INTO_JNI_STACK
	if (self->m_pNativeStackBottom != NULL) {
...
#ifdef FASTIVA_GC_DEBUG
	fastiva_getFieldOffset((u1*)obj, 4);
#endif
...
static inline jobject addLocalReference(ScopedJniThreadState& ts, Object* obj) {
	return addLocalReference(ts.self(), obj);
}

#ifdef FASTIVA
void* fastiva_dvmAddLocalReference(Thread* self, Object* obj) {
	return addLocalReference(self, obj);
}
#endif
...
static jobject addGlobalReference(Object* obj) {
	...
	if (dvmIsClassObject(obj)) {
		return (jobject)((int)obj | kIndirectKindGlobal);
	}
#endif
 ...
 #if FASTIVA_FAST_JNI
	if (true) {
		return reinterpret_cast<jobject>((int)obj | kIndirectKindGlobal);
	}
#endif
   return jobj;
}
 ...
 */
#ifdef FASTIVA
// @zee changes return type: void -> bool
void fastiva_deleteGlobalReference(JNIEnv* env, jobject jobj) {
#else
...
#if FASTIVA_FAST_JNI
	if (((int)jobj & 0x3) != kIndirectKindGlobal) {
		return;
	}
	jobj = (jobject)((int)jobj & ~0x3);
	if (dvmIsClassObject((Object*)(void*)jobj)) {
		return;
    }
#endif
 ...
 */
#ifdef FASTIVA
// @zee fastiva changes return type: bool -> Method*
#endif
static Method* dvmRegisterJNIMethod(ClassObject* clazz, const char* methodName,
...
bool fastiva_dvmCanAllocClass(ClassObject* clazz) {
	return canAllocClass(clazz);
}
...
static void DeleteGlobalRef(JNIEnv* env, jobject jglobalRef) {
    ScopedJniThreadState ts(env);
#ifdef FASTIVA
    fastiva_deleteGlobalReference(env, jglobalRef);
#else
...
static jobject AllocObject(JNIEnv* env, jclass jclazz) {
    ...
    dvmReleaseTrackedAlloc(newObj, ts.self());
	return result;
#else
...
#else
    return (jclass) addLocalReference(ts, (Object*) obj->clazz);
#endif
}
...
#ifdef FASTIVA
#define FASTIVA_VALIDATE_REDIRECTED_METHOD() 
    /*
    @ZEE 2013.0319 methodId always has overriding methodID.
	if (clazz != obj->clazz && meth->accessFlags & ACC_OVERRIDE_RET$) { \
        meth = dvmGetVirtualizedMethod(obj->clazz, (Method*)methodID); \
	}*/
#endif
/*
...
#define CALL_NONVIRTUAL(_ctype, _jname, _retfail, _retok, _isref)           \
    ...
    {                                                                       \
		...
		FASTIVA_VALIDATE_REDIRECTED_METHOD();								\
		va_start(args, methodID);                                           \
        dvmCallMethodV(ts.self(), meth, obj, true, &result, args);          \
        if (_isref && !dvmCheckException(ts.self()))                        \
            result.l = (Object*)addLocalReference(ts.self(), result.l);           \
 ...
 */
#ifdef FASTIVA
static jint fastiva_RegisterNatives(JNIEnv* env, jclass jclazz, const JNINativeMethod* methods, jint nMethods, bool isFNI);
#endif
static jint RegisterNatives(JNIEnv* env, jclass jclazz,
    const JNINativeMethod* methods, jint nMethods)
{
#ifdef FASTIVA
	return fastiva_RegisterNatives(env, jclazz, methods, nMethods, false);
}
...
#ifdef FASTIVA
		Method* method = dvmRegisterJNIMethod(clazz, methods[i].name,
                methods[i].signature, methods[i].fnPtr);
		if (method == NULL) {
            return JNI_ERR;
		}
		else if (isFNI) {
			method->accessFlags |= ACC_FASTIVA_FNI;
		}
#else
...
#if FASTIVA_SCAN_INTO_JNI_STACK
    Object* newObj = dvmAllocObject(bufferClazz, ALLOC_DEFAULT);
    if (newObj == NULL) {
        return NULL;
    }
    jobject result = addLocalReference(ts.self(), newObj);
...
static jint attachThread(JavaVM* vm, JNIEnv** p_env, void* thr_args, bool isDaemon) {
    ...
    if (result) {
        ...
        assert(self != NULL);
#ifdef FASTIVA
		self->m_pNativeStackBottom = NULL;
#endif
...
#ifdef FASTIVA
	self->m_pNativeStackBottom = alloca(8);
#endif
    /* detach the thread */
...
#ifdef FASTIVA
#if !FASTIVA_SCAN_INTO_JNI_STACK
		void* oldSp = self->m_pNativeStackPointer;
...
#ifdef FASTIVA
#if !FASTIVA_SCAN_INTO_JNI_STACK
		self->m_pNativeStackPointer = oldSp;
		dvmChangeStatus(self, oldStatus);
#endif
#else
        dvmChangeStatus(self, THREAD_NATIVE);
...
static jint fastiva_RegisterFastNatives(JNIEnv* env, jclass c, const JNINativeMethod* methods, jint nMethod) {
	return fastiva_RegisterNatives(env, c, methods, nMethod, true);
}

void fastiva_PushExceptionHandler(fastiva_ExceptionContext* exContext) {
	fastiva.pushExceptionHandler(exContext);
}
void fastiva_RemoveExceptionHandler(fastiva_ExceptionContext* exContext) {
	fastiva.removeExceptionHandler(exContext);
}

void fastiva_ReleaseArray(jobject obj) {
	return;
}

typedef void (*FASTIVA_EXCEPTION_CONTEXT_HANDLER_FUNC)(struct fastiva_fniExceptionContext* exContext);
#endif
...
#if 0 //def FASTIVA
    fastiva_RegisterFastNatives,
    fastiva_ReleaseArray,
    (FASTIVA_EXCEPTION_CONTEXT_HANDLER_FUNC)(void*)fastiva_PushExceptionHandler,
    (FASTIVA_EXCEPTION_CONTEXT_HANDLER_FUNC)(void*)fastiva_RemoveExceptionHandler,
#endif
};
...
#ifdef _DEBUG
	//fastiva.debugBreak("JNI_CreateJavaVM", true);
#endif
...
#ifdef FASTIVA
	pVM->fastiva_runtime = &fastiva_RuntimeProxy::g_instance;
#endif
...
#ifdef FASTIVA // TEMP
#ifdef _DEBUG
    ALOGE("DDDD fastiva_check_runtime");
	void fastiva_check_runtime();
	fastiva_check_runtime();
#endif
#ifdef _WIN32
