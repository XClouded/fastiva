...
#include <unistd.h>
#include <string.h>
#if defined HAVE_POSIX_FILEMAP || defined _WIN32
# include <sys/mman.h>
#endif
#include <limits.h>
#include <errno.h>
...
#ifdef _WIN32
 int sysFakeMapFile_fastiva(int fd, MemMapping* pMap, off_t start, size_t length)
#else
...
int sysFakeMapFile(int fd, MemMapping* pMap) {
    ...
    if (getFileStartAndLength(fd, &start, &length) < 0)
        return -1;
	return sysFakeMapFile_fastiva(fd, pMap, start, length);
}
#endif
